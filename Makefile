...
$(LOVE_PATH)/build/Makefile: $(LOVE_PATH)/configure
	...
	ls $(INSTALLPREFIX)
	cd $(LOVE_PATH)/build && CFLAGS="-I$(INSTALLPREFIX)/include" PKG_CONFIG_PATH=$(INSTALLPREFIX)/lib/pkgconfig LDFLAGS="-Wl,-rpath,'\$$\$$ORIGIN/../lib' -L$(INSTALLPREFIX)/lib" lua_CFLAGS="-I$(INSTALLPREFIX)/include" lua_LIBS="-L$(INSTALLPREFIX)/lib/lua/5.4" ../configure --prefix=$(INSTALLPREFIX) --with-lua=lua --with-luaversion=5.4
